...
def fitness_function(schedule):
    ...
    strat_times_pipeline = np.zeros(num_special_orders)
    ...
    for order in schedule:
        ...
        if order_index < num_special_orders:
            ...
            for i in range(num_machines_body):
                if i ==0:
                    if order_index == 0:
                        start_times_body[i, order_index] = release_time
                    else:
                        start_times_body[i, order_index] = max(completion_times_body[i, order_index - 1], release_time)
                else:
                    start_times_body[i, order_index] = start_times_body[i-1, order_index] + processing_times_body[
                        i-1, order_index]
            ...
            for i in range(num_machines_cabinet):
                if i == 0:
                    if order_index == 0:
                        ...
                        start_times_cabinet[i, order_index] = max(completion_times_cabinet[i, order_index - 1],
                                                                  release_time)
                else:
                    start_times_cabinet[i, order_index] = start_times_cabinet[i-1, order_index] + processing_times_cabinet[i-1, order_index]
                    completion_times_cabinet[i, order_index] = start_times_cabinet[i, order_index] + \
                                                               processing_times_cabinet[i, order_index]

            # 计算管道包装的开始时间和完成时间
            strat_times_pipeline[order_index] = max(completion_times_body[:, order_index].max(), completion_times_cabinet[:, order_index].max())
            completion_times_pipeline[order_index] = strat_times_pipeline[order_index] + processing_times_pipeline[order_index]
