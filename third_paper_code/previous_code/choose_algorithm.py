...
due_times_special = np.random.randint(100, 300, num_special_orders)
...
# 创建变量
x = model.addVars(num_special_orders, num_position, vtype=GRB.BINARY, name="x")
...
for j in range(num_special_orders):
    for i in range(num_machines_body):
        ...
        for s in range(num_position - 1):
            for k in range(num_special_orders):
                for v in range(num_special_orders):
                    if k != v:
                        model.addConstr(
                            start_times_body[i, k] >= completion_times_body[i, v] - M * (
                                        1 - x[k, s + 1]) - M * (1 - x[v, s])
                        )
        ...
        for s in range(num_position - 1):
            for k in range(num_special_orders):
                for v in range(num_special_orders):
                    if k != v:
                        model.addConstr(
                            start_times_cabinet[i, k] >= completion_times_cabinet[i, k] - M * (
                                    1 - x[k, s + 1]) - M * (1 - x[v, s])
                        )
...
# 管道包装的时间约束
for j in range(num_special_orders):
    model.addConstr(start_times_pipeline[j] >= completion_times_body[:, j].max())
    model.addConstr(start_times_pipeline[j] >= completion_times_cabinet[:, j].max())
    for s in range(num_position - 1):
        for k in range(num_special_orders):
            for v in range(num_special_orders):
                if k != v:
                    model.addConstr(
                        start_times_pipeline[k] >= completion_times_pipeline[v] - M * (
                                1 - x[k, s + 1]) - M * (1 - x[v, s])
                    )
...
# 完工时间不能超过交期
for j in range(num_special_orders):
    model.addConstr(completion_times_pipeline[j] <= due_times_special[j] + (1 - u[j]) * 1e6)  # 1e6 是一个大数
#计算开始时间
for j in range(num_special_orders):
    start_time[j] = min(start_times_body[0, j]), start_times_cabinet[0, j]


# 特殊订单选择约束
model.addConstrs((gp.quicksum(x[j, i] for j in range(num_special_orders) for i in range(num_position)) == u[j] for j in range(num_special_orders)), "OrderSelection")
